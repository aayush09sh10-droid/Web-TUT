  ...
  return (
    ...
    >
      <div className="mb-3 grid gap-2 sm:inline-flex sm:grid-cols-none">
        <button type="button" onClick={() => dispatch(setHomeField({ field: 'inputMode', value: 'video' }))} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${inputMode === 'video' ? 'border-[var(--text)] bg-[var(--text)] text-[var(--bg)]' : 'border-(--border) bg-(--card) text-(--text)'}`}>
          YouTube Link
        </button>
        <button type="button" onClick={() => dispatch(setHomeField({ field: 'inputMode', value: 'notes' }))} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${inputMode === 'notes' ? 'border-[var(--text)] bg-[var(--text)] text-[var(--bg)]' : 'border-(--border) bg-(--card) text-(--text)'}`}>
          Notes Photo
        </button>
      </div>
      ...
      <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
        {inputMode === 'video' ? (
          ...
          <label className="flex flex-1 cursor-pointer flex-col justify-center rounded-[1.1rem] border border-(--border) bg-(--card-strong) px-4 py-3 text-sm text-(--text) shadow-sm">
            ...
            <span className="mt-1 text-xs text-(--muted)">{notesImage?.fileName || 'Choose a clear image of your handwritten or typed notes.'}</span>
            <input type="file" accept="image/*" onChange={handleNotesFileChange} className="mt-3 text-xs text-(--muted) file:mr-3 file:rounded-full file:border-0 file:bg-[var(--text)] file:px-3 file:py-2 file:text-xs file:font-semibold file:text-[var(--bg)]" />
          </label>
        )}
        ...
        <button type="button" onClick={() => dispatch(setHomeField({ field: 'showComposer', value: false }))} className="inline-flex h-[48px] w-full items-center justify-center rounded-[1.1rem] border border-(--border) bg-(--card-strong) text-base font-semibold text-(--text) shadow-sm transition hover:scale-105 sm:w-[48px]" aria-label="Close paste link box">
          ×
        </button>
      </div>
