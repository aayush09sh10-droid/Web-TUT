      ...
      <section className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-3 pb-10 pt-6 sm:px-4 sm:pt-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Link to="/history" className="rounded-full border border-[var(--border)] bg-[var(--card)] px-4 py-2 text-center text-xs font-medium transition hover:-translate-y-0.5">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[1.9rem] border p-5 backdrop-blur-xl sm:p-6" style={panelStyle}>
            ...
            {loading ? (
              ...
              <p className="mt-3 text-sm text-[var(--muted)]">No topic details are available for this learning session yet.</p>
            )}
              ...
              {wrongQuestions.length > 0 ? (
                <div className="mt-3 space-y-2">
                  {wrongQuestions.map((question) => (
                    <p key={question} className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--text)]">
                      Revise: {question}
                    </p>
                  ))}
                </div>
                ...
                <div className="mt-3 space-y-2">
                  {teachingTopics.slice(0, 4).map((topic) => (
                    <p key={topic.id || topic.title} className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--text)]">
                      Practice more: {topic.title}
                    </p>
                  ))}
                </div>
              ) : (
                <p className="mt-3 text-sm text-[var(--muted)]">Generate teaching or complete a quiz to get your next study targets.</p>
              )}
            </div>
              ...
              {normalizedSummary.timeline.length > 0 && (
                <div className="mt-4 space-y-2 border-b border-[var(--border)] pb-4">
                  ...
                  {normalizedSummary.timeline.map((entry, index) => (
                    <div key={`${entry.timestamp}-${index}`} className="flex flex-col gap-1 text-sm text-[var(--text)] sm:flex-row sm:gap-3">
              ...
              <div className="mt-4 space-y-3">
                {summaryParagraphs.length > 0 ? (
                  summaryParagraphs.map((paragraph, index) => (
                    <p key={index} className="text-sm leading-relaxed text-[var(--text)]">{paragraph}</p>
                  ))
                ) : (
                  <p className="text-sm text-[var(--muted)]">The saved summary text is not available for this lesson.</p>
                )}
              </div>
            </div>
            ...
            <div className="mt-6 rounded-[1.3rem] border border-[var(--border)] bg-[var(--card-strong)] p-4">
              ...
              {item?.result?.formula?.intro && <p className="mt-2 text-sm text-[var(--muted)]">{item.result.formula.intro}</p>}
              {formulaSections.length > 0 ? (
                <div className="mt-4 space-y-3">
                  {formulaSections.map((section, index) => (
                    <div key={section.id || index} className="rounded-[1rem] border border-[var(--border)] bg-[var(--card)] p-4">
                      ...
                      <p className="mt-2 rounded-xl border border-[var(--border)] bg-[var(--card-strong)] px-3 py-2 text-sm text-[var(--text)]">{section.formula}</p>
                      {section.explanation && <p className="mt-3 text-sm leading-relaxed text-[var(--text)]">{section.explanation}</p>}
                      {Array.isArray(section.practiceQuestions) && section.practiceQuestions.length > 0 && (
                        <div className="mt-3 space-y-2">
                          {section.practiceQuestions.map((question, questionIndex) => (
                            <p key={`${section.id || index}-${questionIndex}`} className="rounded-xl border border-[var(--border)] bg-[var(--card-strong)] px-3 py-2 text-sm text-[var(--text)]">
                ...
                <p className="mt-3 text-sm text-[var(--muted)]">No formula guide was saved for this lesson.</p>
              )}
            </div>
          </div>
            ...
            {loading ? (
              ...
              <div className="mt-4 space-y-4">
                {teachingTopics.map((topic, index) => (
                  <div key={topic.id || index} className="rounded-[1.2rem] border border-[var(--border)] bg-[var(--card-strong)] p-4">
                    ...
                    {Array.isArray(topic.notes) && topic.notes.length > 0 && (
                      <div className="mt-3 space-y-2">
                        {topic.notes.map((note, noteIndex) => (
                          <p key={`${topic.id || index}-${noteIndex}`} className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--text)]">
                            {note}
                          </p>
                        ))}
                      </div>
              ...
              <p className="mt-3 text-sm text-[var(--muted)]">No teaching lesson was saved for this topic yet.</p>
            )}
            ...
            <div className="mt-6 rounded-[1.3rem] border border-[var(--border)] bg-[var(--card-strong)] p-4">
              ...
              {loading ? (
                ...
                <>
                  ...
                  <p className="mt-4 text-sm text-[var(--muted)]">Score: {quizProgress?.scorePercent || 0}%</p>
                </>
              ) : (
                <p className="mt-3 text-sm text-[var(--muted)]">No quiz result yet. Complete the quiz from Home to track correct answers here.</p>
              )}
            </div>
