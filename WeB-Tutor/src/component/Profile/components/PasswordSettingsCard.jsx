    ...
    <div className="rounded-[1.9rem] border p-5 backdrop-blur-xl sm:p-6" style={panelStyle}>
      ...
      {showPasswordForm && (
        <form className="mt-5 space-y-3" onSubmit={onSubmit}>
          <input value={passwordForm.currentPassword} onChange={(e) => onUpdateField('currentPassword', e.target.value)} type="password" placeholder="Current password" className="w-full rounded-2xl border border-(--border) bg-[var(--input-bg)] px-4 py-3 text-sm outline-none" />
          <input value={passwordForm.newPassword} onChange={(e) => onUpdateField('newPassword', e.target.value)} type="password" placeholder="New password" className="w-full rounded-2xl border border-(--border) bg-[var(--input-bg)] px-4 py-3 text-sm outline-none" />
          <input value={passwordForm.confirmPassword} onChange={(e) => onUpdateField('confirmPassword', e.target.value)} type="password" placeholder="Confirm new password" className="w-full rounded-2xl border border-(--border) bg-[var(--input-bg)] px-4 py-3 text-sm outline-none" />
